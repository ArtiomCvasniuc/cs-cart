...
{capture name="tab_items"}
    {foreach from=$navigation.tabs item=tab key=key name=tabs}
        {if (!$tabs_section || $tabs_section == $tab.section) && ($tab.hidden || !$key|in_array:$empty_tab_ids)}
            {* check if user is vendor and disable tabs which are not necessary *}
            {if $user_info.user_type == 'V'}
                {if $key != 'plan' && $key != 'subscribers' && $key != 'attachments' && $key != 'addons' && $key != 'reward_points' && $key != 'required_products' && $key != 'tags'}
