...
<div class="control-group profile-field-{$field.field_name}">
    ...
    {elseif $field.field_type == "ProfileFieldTypes::CHECKBOX"|enum}
        ...
        <input type="checkbox" id="{$id_prefix}elm_{$field.field_id}" name="{$data_name}[{$data_id}]" value="Y" {if $value == "Y"}checked="checked"{/if} {$disabled_param nofilter} {if $auth.user_type != 'A'} disabled {/if} /></label>
