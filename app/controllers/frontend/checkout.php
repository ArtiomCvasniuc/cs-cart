...
if ($mode === 'cart') {
    ...
    if(Registry::get("addons.commercial_credit")) {
        $user = fn_get_user_info($cart['user_data']['user_id']);
        if ($user['commercial_credit_enabled'] === 'A') {
            $view->assign("commercial_credit_info", [
                "credit" => $user['commercial_credit'],
                "limit" => $user['commercial_credit_limit']
            ]);
        }
    }
...
// All checkout steps
    ...
    if(Registry::get("addons.commercial_credit")) {
        $usr_data = fn_get_user_info($_SESSION['cart']['user_data']['user_id']);

        if($usr_data['commercial_credit_enabled'] == 'A') {
            $credit = 0.00;
            $limit = 500.00;

            $info = [
                "commercial_credit" => $usr_data['commercial_credit']? $usr_data['commercial_credit'] : $limit,
                "commercial_credit_limit" => $usr_data['commercial_credit_limit'] ? $usr_data['commercial_credit_limit'] : $limit,
            ];
        
            Tygh::$app['view']->assign('commercial_credit_info', $info);
        }
    }
    $view->assign([
        'user_data'              => $cart['user_data'],
