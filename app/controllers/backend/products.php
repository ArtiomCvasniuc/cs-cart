...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
     ...
     */
    if ($mode == 'update') {
        fn_set_hook("test_update_product_pre", $_REQUEST['product_data']);
        $product_id = null;
        if (!empty($_REQUEST['product_data']['product'])) {
            ...
            // requested product min qty
            $product_min_qty = $product_data['min_qty'];
            // get value of min qty feature
            $product_feature_min_qty_id = fn_get_feature_by_id($product_data['product_features']['553']);
            
            // update min qty feature only if requested product min qty is equal or higher than value of min qty feature
            if($product_min_qty >= $product_feature_min_qty_id) {
                fn_update_min_qty_feature($product_id, $product_min_qty);
            }
...
if ($mode == 'add') {
    ...
    fn_set_hook('test_add_product_data_options', $product_data);
...
//
    ...
    /* 
    * sprint_2_promo_text: we don't want the product page to be too flashy,
    * show only essential product information
    */
    unset($product_data['promo_text']);
    /*
    * sprint_2_short_description: short description is not necessary for the product
    */
    unset($product_data['short_description']);
    ...
    fn_set_hook('test_products_update_get_post', $product_data);
