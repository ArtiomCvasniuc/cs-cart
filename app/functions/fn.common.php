...
/**
 * Get feature value (variant) by its id
 * 
 * @param int $id       Represents the feature id
 * 
 * @return int the value (variant)
 */
function fn_get_feature_by_id($id)
{
    $query = db_get_row("SELECT DISTINCT(variant) FROM ?:product_feature_variant_descriptions WHERE variant_id = ?i", $id);
...
/**
 * Update min qty feature
 * 
 * @param int $id       Represents the feature id
 * @param int $min_qty  Represents the requested product's min qty
 */
function fn_update_min_qty_feature($id, $min_qty)
{
    $query = db_get_row("SELECT DISTINCT(?:product_feature_variant_descriptions.variant_id), ?:product_feature_variant_descriptions.variant FROM ?:product_feature_variant_descriptions, ?:product_feature_variants WHERE ?:product_feature_variant_descriptions.variant_id = ?:product_feature_variants.variant_id AND ?:product_feature_variants.feature_id = ?i AND ?:product_feature_variant_descriptions.variant = ?i", '553', $min_qty);
    $variant_id = $query['variant_id'];
    $variant = $query['variant'];

    $query = db_get_row("SELECT count(feature_id) as count FROM ?:product_features_values WHERE feature_id = ?i AND product_id = ?i", '553', $id);
    $count = $query['count'];

    if($min_qty != 0) {
        if($count == 0) {
            $query = db_query("INSERT INTO ?:product_features_values (feature_id, product_id, variant_id, lang_code) VALUES (?i, ?i, ?i, ?s)", '553', $id, $variant_id, 'ro');
            $query = db_query("INSERT INTO ?:product_features_values (feature_id, product_id, variant_id, lang_code) VALUES (?i, ?i, ?i, ?s)", '553', $id, $variant_id, 'en');
        }
        else {
            $query = db_query("UPDATE ?:product_features_values SET variant_id = ?i WHERE feature_id = ?i AND product_id = ?i", $variant_id, '553', $id);
        }
    }
    else {
        if($count > 0) {
            $query = db_query("DELETE FROM ?:product_features_values WHERE feature_id = ?i AND product_id = ?i", '553', $id);
        }
    }
}

/**
 * Update estimated shipping time feature
 * 
 * @param int $company_id                       Represents the id of the company (vendor's id)
 * @param int $vendor_estimated_shipping_time   Represents the vendor's estimated shipping time (from vendor's settings)
 */
function fn_update_estimated_shipping_time_feature($company_id, $vendor_estimated_shipping_time)
{
    $query = db_get_row("SELECT DISTINCT(?:product_feature_variant_descriptions.variant_id), ?:product_feature_variant_descriptions.variant FROM ?:product_feature_variant_descriptions, ?:product_feature_variants WHERE ?:product_feature_variant_descriptions.variant_id = ?:product_feature_variants.variant_id AND ?:product_feature_variants.feature_id = ?i AND ?:product_feature_variant_descriptions.variant = ?i", '555', $vendor_estimated_shipping_time);
    $variant_id = $query['variant_id'];
    $variant = $query['variant'];

    $query = db_get_array("SELECT product_id FROM ?:products WHERE company_id = ?i", $company_id);
    $count = count($query);
    
    for($i=0; $i<$count; $i++) {
        $product_id = $query[$i]['product_id'];

        $query_count = db_get_row("SELECT count(feature_id) as count FROM ?:product_features_values WHERE product_id = ?i AND feature_id = ?i", $product_id, '555');

        if($vendor_estimated_shipping_time != '') {
            if($query_count['count'] == 0) {
                $query_insert = db_query("INSERT INTO ?:product_features_values (feature_id, product_id, variant_id, lang_code) VALUES (?i, ?i, ?i, ?s)", '555', $product_id, $variant_id, 'ro');
                $query_insert = db_query("INSERT INTO ?:product_features_values (feature_id, product_id, variant_id, lang_code) VALUES (?i, ?i, ?i, ?s)", '555', $product_id, $variant_id, 'en');
            }
            else {
                $query_update = db_query("UPDATE ?:product_features_values SET variant_id = ?i WHERE feature_id = ?i AND product_id = ?i", $variant_id, '555', $product_id);
            }
        }
        else {
            $query_delete = db_query("DELETE FROM ?:product_features_values WHERE feature_id = ?i AND product_id = ?i ", '555', $product_id);
        }
    }
}
...
/**
 * Get value from vendor's settings which explains if vendor wants to show product price for not authorized users
 * 
 * @param int $vendor_id    Represents the vendor id (company id)
 * 
 * @return int $value
 */
function fn_is_necessary_to_show_price($vendor_id)
{
    $query = db_get_row("SELECT user_id FROM ?:users WHERE company_id = ?i", $vendor_id);
    $user_id = $query['user_id'];

    $query = db_get_row("SELECT show_product_price_only_for_auth_users FROM ?:user_profiles WHERE user_id = ?i", $user_id);
    $value = $query['show_product_price_only_for_auth_users'];
    
    return $value;
}

// sprint_8_first_order_free_shipping
/**
 * Get orders count by user id
 * 
 * @param int $user_id      Represents the user's id value
 * 
 * @return int $count
 * 
 */
function fn_get_orders_count($user_id) 
{
    $query = db_get_row("SELECT COUNT(order_id) as count FROM ?:orders WHERE user_id = ?i", $user_id);
    $count = $query['count'];
...
/**
 * Check if first order should be with free shipping
 * 
 * @param int $vendor_id        Represents the vendor's id value (company id value)
 * 
 * @return char $value
 */
function fn_free_shipping_first_order($vendor_id)
{
    $query = db_get_row("SELECT field_id FROM ?:profile_fields WHERE field_name = 'first_order_free_shipping'");
    $field_id = $query['field_id'];

    $query = db_get_row("SELECT value FROM ?:profile_fields_data WHERE object_id = ?i AND field_id = ?i", $vendor_id, $field_id);
    $value = $query['value'];
