...
function fn_change_order_status($order_id, $status_to, $status_from = '', $force_notification = array(), $place_order = false)
{
    ...
    // sprint_8_email_notifications: we don't want to send emails to the buyer with vendor data
    $order_info['company_id'] = '0';
            ...
            foreach ($group['shippings'] as $shipping_id => $shipping) {
                if (isset($cart['chosen_shipping'][$key_group]) && $cart['chosen_shipping'][$key_group] == $shipping_id) {
                    ...
                    if($auth['user_id'] != '') {
                        ...
                        $count_orders = fn_get_orders_count($user_id);
                        if($count_orders == 0) {
                            $free_shipping_first_order = fn_free_shipping_first_order($company_id);
                            if($free_shipping_first_order == 'Y') {
                                $shipping['rate'] = 0;
                            }
                        }
                    }
